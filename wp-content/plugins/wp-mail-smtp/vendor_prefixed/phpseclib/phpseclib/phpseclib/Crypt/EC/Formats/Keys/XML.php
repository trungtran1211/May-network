...
abstract class XML
{
     ...
     * @return object[]
     */
    private static function extractPointRFC4050(\DOMXPath $xpath, \WPMailSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Base $curve)
    {
        ...
        $point = [$curve->convertInteger(new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger($x->item(0)->getAttribute('Value'))), $curve->convertInteger(new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger($y->item(0)->getAttribute('Value')))];
    ...
    private static function loadCurveByParam(\DOMXPath $xpath)
    {
        ...
        if ($namedCurve->length == 1) {
            ...
            if ($name === \false) {
                throw new \WPMailSMTP\Vendor\phpseclib3\Exception\UnsupportedCurveException('Curve with OID of ' . $oid . ' is not supported');
            }
            $curve = '\\WPMailSMTP\\Vendor\\phpseclib3\\Crypt\\EC\\Curves\\' . $name;
            if (!\class_exists($curve)) {
                throw new \WPMailSMTP\Vendor\phpseclib3\Exception\UnsupportedCurveException('Named Curve of ' . $name . ' is not supported');
            }
            return new $curve();
        }
        ...
        switch ($type) {
            case 'prime-field':
                ...
                $curve->setModulo(new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger($p, 256));
                $curve->setCoefficients(new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger($a, 256), new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger($b, 256));
                $point = self::extractPoint("\0" . $base, $curve);
                $curve->setBasePoint(...$point);
                $curve->setOrder(new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger($order, 256));
                return $curve;
            case 'gnb':
            case 'tnb':
            case 'pnb':
            default:
                throw new \WPMailSMTP\Vendor\phpseclib3\Exception\UnsupportedCurveException('Field Type of ' . $type . ' is not supported');
        }
    }
    /**
     * Returns an instance of \phpseclib3\Crypt\EC\BaseCurves\Base based
    ...
    private static function loadCurveByParamRFC4050(\DOMXPath $xpath)
    {
        ...
        switch ($type) {
            case 'prime-field':
                $curve = new \WPMailSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Prime();
                $p = \str_replace(["\r", "\n", ' ', "\t"], '', $p);
                $curve->setModulo(new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger($p));
                $a = \str_replace(["\r", "\n", ' ', "\t"], '', $a);
                $b = \str_replace(["\r", "\n", ' ', "\t"], '', $b);
                $curve->setCoefficients(new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger($a), new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger($b));
                $x = \str_replace(["\r", "\n", ' ', "\t"], '', $x);
                $y = \str_replace(["\r", "\n", ' ', "\t"], '', $y);
                $curve->setBasePoint(new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger($x), new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger($y));
                $order = \str_replace(["\r", "\n", ' ', "\t"], '', $order);
                $curve->setOrder(new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger($order));
                return $curve;
            default:
                throw new \WPMailSMTP\Vendor\phpseclib3\Exception\UnsupportedCurveException('Field Type of ' . $type . ' is not supported');
        }
     ...
     * @return string|false
     */
    private static function encodeXMLParameters(\WPMailSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Base $curve, $pre, array $options = [])
    {
