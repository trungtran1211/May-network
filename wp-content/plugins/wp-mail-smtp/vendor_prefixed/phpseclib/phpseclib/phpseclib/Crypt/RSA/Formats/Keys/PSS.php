...
abstract class PSS extends \WPMailSMTP\Vendor\phpseclib3\Crypt\Common\Formats\Keys\PKCS8
{
    ...
    public static function load($key, $password = '')
    {
        ...
        if (isset($key[$type . 'KeyAlgorithm']['parameters'])) {
            $decoded = \WPMailSMTP\Vendor\phpseclib3\File\ASN1::decodeBER($key[$type . 'KeyAlgorithm']['parameters']);
            if ($decoded === \false) {
                throw new \UnexpectedValueException('Unable to decode parameters');
            }
            $params = \WPMailSMTP\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], \WPMailSMTP\Vendor\phpseclib3\File\ASN1\Maps\RSASSA_PSS_params::MAP);
        } else {
            $params = [];
        }
        if (isset($params['maskGenAlgorithm']['parameters'])) {
            $decoded = \WPMailSMTP\Vendor\phpseclib3\File\ASN1::decodeBER($params['maskGenAlgorithm']['parameters']);
            if ($decoded === \false) {
                throw new \UnexpectedValueException('Unable to decode parameters');
            }
            $params['maskGenAlgorithm']['parameters'] = \WPMailSMTP\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], \WPMailSMTP\Vendor\phpseclib3\File\ASN1\Maps\HashAlgorithm::MAP);
        } else {
     ...
     * @return string
     */
    public static function savePublicKey(\WPMailSMTP\Vendor\phpseclib3\Math\BigInteger $n, \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger $e, array $options = [])
    {
        self::initialize_static_variables();
        $key = \WPMailSMTP\Vendor\phpseclib3\Crypt\RSA\Formats\Keys\PKCS1::savePublicKey($n, $e);
        $key = \WPMailSMTP\Vendor\phpseclib3\File\ASN1::extractBER($key);
        $params = self::savePSSParams($options);
    ...
    public static function savePSSParams(array $options)
    {
        ...
        */
        ...
        if (isset($options['MGFHash'])) {
            ...
            $temp = \WPMailSMTP\Vendor\phpseclib3\File\ASN1::encodeDER($temp, \WPMailSMTP\Vendor\phpseclib3\File\ASN1\Maps\HashAlgorithm::MAP);
            $params['maskGenAlgorithm'] = ['algorithm' => 'id-mgf1', 'parameters' => new \WPMailSMTP\Vendor\phpseclib3\File\ASN1\Element($temp)];
        }
        if (isset($options['saltLength'])) {
            $params['saltLength'] = new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger($options['saltLength']);
        }
        return new \WPMailSMTP\Vendor\phpseclib3\File\ASN1\Element(\WPMailSMTP\Vendor\phpseclib3\File\ASN1::encodeDER($params, \WPMailSMTP\Vendor\phpseclib3\File\ASN1\Maps\RSASSA_PSS_params::MAP));
    }
}
