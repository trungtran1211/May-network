...
class Integer extends \WPMailSMTP\Vendor\phpseclib3\Math\Common\FiniteField\Integer
{
     ...
     * @return static
     */
    public function pow(\WPMailSMTP\Vendor\phpseclib3\Math\BigInteger $x)
    {
        $temp = new static($this->instanceID);
    ...
    public function squareRoot()
    {
        ...
        $q = clone $p_1;
        $s = \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger::scan1divide($q);
        ...
        $m = new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger($s);
    ...
    {
        $w++;
        $mask = new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger((1 << $w) - 1);
        $sub = new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger(1 << $w);
